...
scanRangesLengthCorrectionFactor = 3.1861
...
def clbk_laser(msg): #callback para lidar
    ...
    #Definiendo las zonas en grados. Estos son mejorables
...
def take_action(regions):
    ...
    if regions['front'] > d and regions['fright'] > 2*d and regions['right'] > 2*d and regions['bright'] > 2*d:
        state_description = 'case 1 - nothing' #No hay nada, goes hacia adelante (Inicio)
        linear_x = vx * vf
        angular_z = 0
    elif regions['front'] < d: # te encuentras un obstaculo delante
        ...
        linear_x = 0
        angular_z = +wz/2* vf
    elif regions['front'] > d and regions['right'] < d: # seguir con la pared a la derecha
        state_description = 'case 4 - right'
        linear_x = vx* vf
        angular_z = 0
    elif regions['bright'] < d: # No perder la pared (Problema si esquina concava)
        state_description = 'case 5 - bright'
        linear_x = 0
        angular_z = -wz/2* vf
    else: #Hemos perdido la pared
